...
class VersionsMapper extends QBMapper {
	...
	public function deleteAllVersionsForUser(string $userId, string $path = null): int {
		...
		$filesVersionSelect->select('fileid')
			...
			->where($filesVersionSelect->expr()->like('f.path', $deleteQuery->createNamedParameter('files' . ($path ? '/' . $this->db->escapeLikeParameter($path) : '') . '/%', IQueryBuilder::PARAM_STR)))			->andWhere($filesVersionSelect->expr()->eq('m.user_id', $deleteQuery->createNamedParameter($userId, IQueryBuilder::PARAM_STR)))
